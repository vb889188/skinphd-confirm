...
export async function loadRemoteWorkspace(): Promise<Pick<WorkspaceState, "branches" | "people" | "templates" | "agreements" | "signatures" | "links" | "audit" | "records">> {
  const [clinics, people, agreements, signatures, links, audit, uploaded, records] = await Promise.all([
    ...
    rest<{ id: string; person_id: string; file_name: string; mime_type: string; byte_size: number; sha256: string; note: string; extracted_text?: string; created_at: string }[]>(
...
export async function fetchSourceFile(id: string) {
  const rows = await rest<Array<{ file_name: string; mime_type: string; content_base64: string }>>(
    `confirm_source_files?id=eq.${encodeURIComponent(id)}&select=file_name,mime_type,content_base64`,
  );
  return rows[0] ?? null;
}
